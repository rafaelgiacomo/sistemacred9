        ...
        static void Main(string[] args)
        {
            ...
            try
            {
                ...
                var response = contratoNegocio.RealizarImportacao(TipoPlanilhaEnum.Outros,
                                                                                   arquivo,
                                                                                   arquivo.Replace(Directory.GetCurrentDirectory() + "\\PlanilhasImportacao\\", ""));
        ...
        private static string SelecionarArquivoImportacao()
        {
            ...
            while (!nomeArquivook)
            {
                try
                {
                    ...
                    string[] arquivos = Directory.GetFiles(Directory.GetCurrentDirectory() + "\\PlanilhasImportacao");

                    int cont = 0;
                    foreach (var arq in arquivos)
                    {
                        cont++;
                        Console.WriteLine(cont + " - " + arq.Replace(Directory.GetCurrentDirectory() + "\\PlanilhasImportacao\\", ""));
                    }
